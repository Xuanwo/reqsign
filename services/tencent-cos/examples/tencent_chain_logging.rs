...
use reqsign_tencent_cos::{ConfigCredentialProvider, Credential, DefaultCredentialProvider};
use std::sync::Arc;
...
async fn main() -> Result<()> {
    ...
    let chain = ProvideCredentialChain::new()
        .push(LoggingProvider::new(
            "Config",
            ConfigCredentialProvider::new(Arc::new(reqsign_tencent_cos::Config::default())),
        ))
        .push(LoggingProvider::new(
            "Default",
            DefaultCredentialProvider::new(reqsign_tencent_cos::Config::default()),
        ));
