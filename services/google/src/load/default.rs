...
impl ProvideCredential for DefaultLoader {
    ...
    async fn provide_credential(&self, ctx: &Context) -> Result<Option<Self::Credential>> {
        ...
        let raw_cred = config_loader.provide_credential(ctx).await?;
        ...
        if let Some(raw_cred) = raw_cred {
            ...
            if let Some(sa) = raw_cred.service_account {
                ...
                if self.config.scope.is_some() {
                    ...
                    if let Some(token) = loader.provide_credential(ctx).await? {
        ...
        if !self.config.disable_env {
            ...
            if let Some(token) = vm_loader.provide_credential(ctx).await? {
                return Ok(Some(Credential::Token(token)));
            }
        }
