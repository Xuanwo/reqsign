...
impl<T> SignableRequest for http::Request<T> {
    fn build(&mut self) -> Result<SigningContext> {
        let (mut parts, _) = http::request::Request::new(()).into_parts();
        parts.method = self.method().clone();
        parts.uri = mem::take(self.uri_mut());
        parts.version = self.version();
        parts.headers = mem::take(self.headers_mut());
        parts.extensions = mem::take(self.extensions_mut());
        parts.build()
    }

    fn apply(&mut self, ctx: SigningContext) -> Result<()> {
        let (mut parts, _) = http::request::Request::new(()).into_parts();
        parts.method = self.method().clone();
        parts.uri = mem::take(self.uri_mut());
        parts.version = self.version();
