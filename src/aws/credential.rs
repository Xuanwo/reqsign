...
impl Loader {
    ...
    async fn load_via_assume_role(&self, cred: Credential) -> Result<Option<Credential>> {
        ...
        let mut url = format!("https://{endpoint}/");
        ...
        let mut req = self
            ...
            .body(format!("Action=AssumeRole&DurationSeconds={duration_seconds}&RoleArn={role_arn}&RoleSessionName={role_session_name}&Version=2011-06-15"))
            .build()?;
        ...
        temp_env::with_vars(
            ...
            || {
                RUNTIME.block_on(async {
                    ...
                    let loader =
                        Loader::new(reqwest::Client::new(), config).with_disable_ec2_metadata();
        ...
        temp_env::with_vars(
            ...
            || {
                RUNTIME.block_on(async {
                    ...
                    config.assume_role_arn = Some(assume_role_arn.clone());
                    let loader =
                        Loader::new(reqwest::Client::new(), config).with_disable_ec2_metadata();
